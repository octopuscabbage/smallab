...
class SimpleExperiment(OverlappingOutputCheckpointedExperiment):
    ...
    def step(self):
        ...
        if self.i in self.num_calls:
            ...
            return OverlappingOutputCheckpointedExperimentReturnValue(should_continue,specification,result,progress,max_iterations)
        else:
...
runner.run(name, specifications, SimpleExperiment(),specification_runner=MultiprocessingRunner(),use_dashboard=False,propagate_exceptions=True)
